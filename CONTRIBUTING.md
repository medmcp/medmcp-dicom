...
**Follow the existing conventions.** Match the patterns the current tools already use — for example, every tool that shells out to `dcm2niix` resolves it through `find_dcm2niix()` rather than assuming it is on `PATH`, so a missing binary fails with an install hint instead of an obscure error from deeper in the call. When in doubt, copy the closest existing tool.
